...
WIN_Play_Samples ( const void* data, size_t len )
{
	...
	do {
		while(PlayedWaveHeadersCount > 0 ) free_memory();               // free used blocks ...
		if(ScheduledBlocks < sizeof(PlayedWaveHeaders)/sizeof(*PlayedWaveHeaders)) break; // wait for a free block ...			
		Sleep(20); //26
	}while(1);
