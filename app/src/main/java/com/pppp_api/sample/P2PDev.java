                ...
                if (nRecvSize[0] > 0) {
                    ...
                    if (nRecvSize[0] > 0) {
                        ...
                        if (nRecvSize[0] >= MAX_SIZE_BUF)
                            System.out.println("====nRecvSize>64*1024, nCurStreamIOType=" + nCurStreamIOType);
                        if (nCurStreamIOType == AVStreamIO_Proto.SIO_TYPE_AUDIO)
                            m_fifoAudio.addLast(pAVData, nRecvSize[0]);
                        else if (nCurStreamIOType == AVStreamIO_Proto.SIO_TYPE_VIDEO)
                            myDoVideoData(m_handleSession, pAVData, nRecvSize[0]);
                    }
                }
            } while (m_bRunning);
            ...
            switch (stFrameHead.getCodecID()) {
                case AVStreamIO_Proto.CODECID_V_H264:
                    if (m_nInitH264Decoder >= 0) {
                        if (bFirstFrame_video && stFrameHead.getFlag() != AVStreamIO_Proto.VFRAME_FLAG_I)
                            break;
                        ...
                        while (nFrameSize > 0) {
                            ...
                            if (out_4para[0] > 0) {
                                if (out_4para[2] > 0 && out_4para[2] != bmpWidth) {
                                    if (bmpWidth != 0)
                                        H264Codec.H264Decode(out_bmp565, pAVData, nFrameSize, out_4para);
                            ...
                            if (nFrameSize > 0)
                                System.arraycopy(pAVData, consumed_bytes, pAVData, 0, nFrameSize);
                            else nFrameSize = 0;
                        }
                    }
                    break;
